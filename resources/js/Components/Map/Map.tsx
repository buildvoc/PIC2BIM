...
function Map({ data, onClick, isSelected,isUnassigned,  zoomFilter}: any) {
    ...
    const loadClustersAndImage = () => {
        const coordintates = data.map((task: any) => task.location);
                      ...
                      (error:any, image: any) => {
                        if (error) {
                          reject(error);
                            ...
                            mapRef.current.addImage(`marker-${status}`, image);
                          resolve("success");
                        }
                      }
                    );
